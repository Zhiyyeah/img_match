...
def load_satellite_data(goci_file, landsat_file):
    ...
    with nc.Dataset(goci_file, 'r') as dataset:
        ...
        goci_data['lon'] = nav_group.variables['longitude'][:].astype(np.float16)
        goci_data['lat'] = nav_group.variables['latitude'][:].astype(np.float16)
        ...
        for g_band, l_band in band_pairs:
            ...
            if var_name in rrs_group.variables:
                goci_data[var_name] = rrs_group.variables[var_name][:].astype(np.float16)
    ...
    with nc.Dataset(landsat_file, 'r') as dataset:
        # 读取经纬度
        landsat_data['lon'] = dataset.variables['lon'][:].astype(np.float16)
        landsat_data['lat'] = dataset.variables['lat'][:].astype(np.float16)
        ...
        for g_band, l_band in band_pairs:
            ...
            if var_name in dataset.variables:
                landsat_data[var_name] = dataset.variables[var_name][:].astype(np.float16)
...
def visualize_upsampling_comparison(goci_data, landsat_data, upsampled_data, band_pair):
    ...
    for title, data, var_name, idx in datasets:
        ...
        if var_name in data:
            ...
            # 设置经纬度范围限制
            ax.set_xlim(120.45, 120.65)  # 经度范围：120-121.2
            ax.set_ylim(26.85, 27.05)  # 纬度范围：26.8-27.8
            '''
